...
/* ---------- CONSTANTS ---------- */
const DEV_MODE = true; // To automatically log in after server refresh
const DEV_USER = {
...
router.get('/edit/:id/:batch/:page', (req, res) => {
    ...
    if (req.session.loggedIn) {
        res.render('users/edit', {name: req.session.name});
    } else {
        res.render('landing', {attempt: req.query.attempt});
    }
});
...
router.get('/payment', (req, res) => {
    ...
    if (req.session.loggedIn) {
        res.render('users/payment', {name: req.session.name});
    } else {
        res.render('landing', {attempt: req.query.attempt});
    }
});
...
router.get('/records', (req, res) => {
    ...
    if (req.session.loggedIn) {
        res.render('users/records', {name: req.session.name});
    } else {
        res.render('landing', {attempt: req.query.attempt});
    }
});

router.get('/records/page/:id', (req, res) => {
    logCall(req.route.path);
